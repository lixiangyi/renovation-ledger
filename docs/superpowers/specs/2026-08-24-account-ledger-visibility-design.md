...
| 未登录 | 仅展示未绑定本地账本；不加「（本地）」。已绑定本在退出时硬删（见 `2026-08-25-logout-purge-bound-ledgers-design.md`） |
| 双端 | Android + 小程序同步改 |
...
- 不改邀请加入流程（加入成功仍为账号云本）  
- 退出登录时硬删已绑定本机副本（保留未绑定本）；详见 `2026-08-25-logout-purge-bound-ledgers-design.md`
